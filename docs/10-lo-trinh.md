...
| Đa typology & đa vùng | ✅ **TRỌN LỘ TRÌNH P6–P9 xong 14/07/2026** ([doc 12](12-mo-rong-typology.md)): rule-pack là dữ liệu; mái dốc + đa mặt tiền + địa hình; thư viện 6 template (nhà ống, biệt thự, cấp 4, nhà vườn, căn hộ, UK detached); lớp region (UK Approved Documents đầu tiên, estimate đa tiền tệ, cờ ⚠ theo pack). Contributor: [doc 13](13-viet-rule-pack.md) + `CONTRIBUTING.md`. 247 tests | xong |
| ~~Đóng gói cài đặt~~ | ✅ 14/07/2026 — npm `atelier-mcp` (packages/cli): esbuild bundle server+core thành MỘT file (playwright external), kèm web editor đã build + font Việt trong gói; `ATELIER_DIR` mặc định = thư mục đang đứng → người dùng KHÔNG clone repo, chỉ `claude mcp add atelier -- npx -y atelier-mcp` trong thư mục nhà của họ; bảng đơn giá địa phương đọc từ `<dự án>/rules/don-gia.json` đè bảng gói (setDonGia); smoke test `packages/cli/smoke.mts` chạy đúng bản cài tarball trước khi publish | xong |
