...
type Project = {
  ...
  config?: ProjectConfig;          // P6 (doc 12): region/typology/rule-pack — bỏ trống = bộ VN
  site: Site;
  ...
  roofs?: Roof[];                  // mái DỐC (P7) — file cũ thiếu trường này vẫn hợp lệ
  stairs: Stair[];
  rooms: Room[];
  furniture: Furniture[];
...
type ProjectConfig = {             // P6 — doc 12/13
  region?: string;                 // bỏ trống = "vn" → bộ pack chuẩn VN như cũ
  typology?: string;               // "nha-ong" | "biet-thu"… — mở khóa rule gắn typologies
  packs?: string[];                // ghi đè tường minh danh sách pack (core geo luôn chạy)
};

type Site = {
  boundary: [number, number][];    // polygon — chấp nhận đất méo
  north: number;                    // độ lệch bắc so với y+
  front: number;                    // cạnh mặt tiền CHÍNH (index cạnh của boundary) — mặt đứng chiếu cạnh này
  setbacks?: { front?: number; back?: number; left?: number; right?: number };
  edges?: SiteEdge[];              // P7: thuộc tính TỪNG cạnh — lô góc nhiều mặt tiền
  terrain?: { elevations: number[] }; // P7: cao độ MẶT ĐẤT tại từng đỉnh boundary (mm so ±0.000,
                                   // âm = đất thấp hơn nền trệt); bỏ trống = đất phẳng z=0.
                                   // Trong lô nội suy IDW (groundAt).
};

type SiteEdge = {                  // cạnh boundary[i] → boundary[i+1]
  kind?: "street" | "alley" | "neighbor"; // street/alley = mặt thoáng (PLN-06 cho vươn ô văng/mái)
  setback?: number;                // khoảng lùi RIÊNG cạnh này (mm) — PLN-07 kiểm
};
...
/**
 * Mái DỐC (P7 — doc 12). Quy ước v1:
 * - level = tầng TRÊN CÙNG mà mái che; đáy diềm ở z = level.elevation + level.height.
 * - outline = hình chiếu bằng MÉP NGOÀI mái, ĐÃ GỒM đua (overhang chỉ là metadata).
 * - pitch = độ dốc theo ĐỘ (phổ biến 18–34°). Mái bằng BTCT vẫn là Slab roof-flat.
 * - gable: nóc dọc ridgeAxis qua tâm bbox (2 mặt); shed: 1 mặt, cạnh cao phía highSide;
 *   hip: 4 mặt trên bbox — outline nên là CHỮ NHẬT, polygon phức tạp cần straight skeleton (v2).
 * - Một nguồn hình học cho mọi renderer: geometry/roof.ts (roofGeometry + roofProfile).
 */
type Roof = {
  id: string; level: string;
  kind: "gable" | "hip" | "shed";
  outline: [number, number][];
  pitch: number;                                   // độ
  ridgeAxis?: "x" | "y";                           // bỏ trống = trục DÀI bbox
  highSide?: "min" | "max";                        // chỉ shed; mặc định "min"
  overhang?: number;                               // mm — metadata (outline đã gồm đua)
  thickness?: number;                              // dày kết cấu vuông góc mặt mái; mặc định 150
  material?: string;                               // "ngoi" | "ton" — nuôi estimate (mai_ngoi 0.7 / mai_ton 0.3)
};
