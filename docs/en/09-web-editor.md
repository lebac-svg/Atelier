...
- UI language: Vietnamese first (informal "bạn" address), with an i18n skeleton in place. ✅ *2026-07-13: the i18n framework is built — all UI chrome + the 106 asset names are bilingual vi/en. The language AUTO-DETECTS BY COUNTRY: a machine in Vietnam (timezone) or a Vietnamese-language browser → vi, elsewhere → en (no geo-IP — timezone + browser language, privacy-friendly); a manual choice via the VI/EN button or `?lang=` always wins and is remembered. v1 limits: exported drawings and server-broadcast validator/summary messages stay Vietnamese (drawings are TCVN documents; messages are shared across all clients).*
