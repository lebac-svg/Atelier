...
> ✅ **P9 hoàn thành 14/07/2026.** (1) **RegionDef registry** trong core: `{id, title, units, packs (theo thứ tự chạy), currency?}` — `registerRegion`/`getRegion`/`listRegions`; `packsFor` đọc RegionDef (region lạ fallback match `pack.region` như cũ); region `vn` đóng gói lại nguyên trạng đúng thứ tự [std-vn, loban, pln] (golden bất động), mô-đun phong tục nằm trong `packs` (kind customs đã ở pack header). (2) **Pack region thật đầu tiên: `uk-approved-docs`** (5 rules: NDSS diện tích ngủ, Part M hành lang + cửa, Part K thang, lấy sáng) — chọn UK theo Phụ lục A2 (văn bản mở OGL); TOÀN BỘ `verified:false` trung thực (đặt theo trí nhớ chuẩn ngành, chưa đối chiếu PDF gốc — contributor UK xác nhận theo doc 13); cờ ⚠ chân tờ giờ theo ĐÚNG bộ pack của project (`unverifiedRules(p)` + option `unverified` của khung tên). (3) **Estimate đa region**: `donGiaFor(region)` — vn giữ nguyên; region chưa có bảng → chỉ khối lượng + ghi chú mời đóng góp, **không một đồng VND lạc region**; tờ KT dự toán tự rút khỏi bộ hồ sơ (nếp mặt cắt-không-thang); `DonGia.currency` sẵn cho bảng cộng đồng. (4) **Fixture DoD `detached-uk`** (2 tầng gable 35°, thang thẳng riser 207.7 đạt Part K) — validate chỉ ra GEO+UK (0 STD/LBB/PLN), estimate 0 VND, template `detached-uk` đăng ký; sửa nốt bias STD-05 cùng họ P7 (cửa chính nhà lùi trong lô nhận đúng loại — helper `isOutdoorPoint` dùng chung STD-05/09). (5) **CONTRIBUTING.md** gốc repo: checklist 3 loại đóng góp (pack/template/region) + doc 13 thêm mục Region (VN+EN). **247 tests xanh.** *Điều chỉnh so spec:* "đóng góp mẫu đi trọn quy trình" chính là pack UK + fixture detached-uk (tự ăn dogfood quy trình contributor); nạp pack động từ thư mục dự án chưa cần — registry tĩnh đủ cho builtin, để mở khi có PR cộng đồng thật; ft-in display + bảng giá UK + i18n nhãn bản vẽ ghi backlog.
