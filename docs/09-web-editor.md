...
- Ngôn ngữ UI: tiếng Việt trước (xưng "bạn"), i18n để sẵn khung. ✅ *13/07/2026: khung i18n đã dựng — toàn bộ UI chrome + tên 106 asset song ngữ vi/en. Ngôn ngữ TỰ NHẬN THEO QUỐC GIA: máy ở Việt Nam (múi giờ) hoặc trình duyệt tiếng Việt → vi, ngoài Việt Nam → en (không geo-IP — múi giờ + ngôn ngữ trình duyệt, riêng tư); chọn tay bằng nút VI/EN hoặc `?lang=` luôn thắng auto và được nhớ. Giới hạn v1: bản vẽ xuất ra và message validator/summary từ server giữ tiếng Việt (hồ sơ là tài liệu TCVN; message broadcast chung mọi client).*
