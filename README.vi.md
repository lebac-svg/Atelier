...
**Đa loại nhà, đa vùng từ trong thiết kế.** Atelier khởi đầu với **nhà ống Việt Nam** — ký hiệu TCVN, quy tắc kích thước VN, thước Lỗ Ban — và nay phủ **sáu loại nhà trên mọi kiểu đất**: nhà ống, biệt thự mái hip trên lô góc đất dốc, nhà cấp 4, nhà vườn, cải tạo căn hộ, và detached kiểu Anh (kiểm theo pack **UK Approved Documents**). Chuẩn xây dựng là **rule pack cắm được** (TCVN đi đầu, pack cộng đồng luôn được chào đón — xem [CONTRIBUTING](CONTRIBUTING.md)); mái dốc, mặt tiền theo từng cạnh lô và địa hình dốc là công dân hạng nhất trong schema.
...
## Demo 50 giây — từ đất trống tới biệt thự có hồ sơ

▶ **[Xem video](docs/media/demo-tu-dau-den-cuoi.webm)** (50s, webm) — một cú quay liền mạch không cắt ghép: brief phỏng vấn → tầng trệt mọc từng nhịp trên mặt bằng 2D → chuyển 3D xem tầng lầu và **mái hip 30°** lợp lên trên **nền đất dốc** → orbit → kê nội thất từ catalog → **đi bộ trong nhà** → sun study 8h→17h → validator sạch, xuất bộ hồ sơ 6 tờ. Tự tái tạo: `npx tsx packages/server/scripts/demo-video.ts`.

| Mặt bằng mọc live (chia đôi) | Mái hip trên lô góc đất dốc | Đi bộ gian khách | Sun study 17:00 |
|---|---|---|---|
| ![Dựng 2D](docs/media/demo-1-dung-2d.png) | ![Mái hip 3D](docs/media/demo-2-mai-hip-3d.png) | ![Đi bộ](docs/media/demo-3-di-bo.png) | ![Sun study](docs/media/demo-4-sun-study.png) |
...
✅ **Đa loại nhà & đa vùng — P6–P9 ([doc 12](docs/12-mo-rong-typology.md), 14/07/2026):** **rule pack là dữ liệu** (chuẩn xây dựng thành gói JSON cắm được, chọn theo `config.region`/`typology`; lõi hình học luôn chạy; [tự viết pack](docs/13-viet-rule-pack.md)) · **mái dốc** (entity gable/hip/shed — một nguồn hình học nuôi mặt bằng/mặt đứng/mặt cắt/3D/GLB/**IfcRoof**/dự toán theo m² mặt nghiêng thật) · **mặt tiền theo từng cạnh lô** (lô góc, khoảng lùi từng cạnh — PLN-07) · **địa hình dốc** (cao độ đất theo đỉnh ranh: đường đất dốc trên mặt đứng/mặt cắt, đất 3D nhấp nhô, chiều cao đo từ cốt vỉa hè) · **thư viện 6 template** (nhà ống, biệt thự, cấp 4, nhà vườn, căn hộ, UK detached — mỗi cái một fixture golden-test) · **lớp region** (đơn vị + bộ pack + tiền tệ; **UK Approved Documents** là pack ngoài VN đầu tiên, cờ ⚠ trung thực tới khi từng số được đối chiếu văn bản; dự toán không bao giờ lạc tiền tệ vùng khác).
...
| 12 | [Mở rộng đa typology](docs/12-mo-rong-typology.md) | P6–P9: rule pack, mái/địa hình, template, region + phụ lục nghiên cứu |
| 13 | [Viết rule pack](docs/13-viet-rule-pack.md) | Hướng dẫn contributor: schema pack, trích nguồn, region |

## Stack

TypeScript monorepo — Node.js (MCP server + WebSocket), Three.js (3D), SVG (bản vẽ 2D), Vite (web editor), Vitest + Playwright (247 tests, gồm e2e trình duyệt thật). Chi tiết và lý do: `03-kien-truc-he-thong.md`.
