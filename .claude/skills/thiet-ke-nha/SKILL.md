...
4. Tự nhìn trước khi mời xem: sau mỗi thay đổi lớn → `validate` + `render_plan` và TỰ soi ảnh trước khi đưa người dùng.
